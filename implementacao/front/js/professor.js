...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function enviarMoedas(e) {
    ...
    try {
      ...
      const alunoData = await api.get(
        '/usuarios/perfil/' + encodeURIComponent(alunoEmail) + '?tipoUsuario=ALUNO'
      );
      ...
      // Envio de moedas
      await api.post('/professor/enviar-moedas', { alunoId, valor, mensagem });

      enviarMoedasMsg.textContent = 'Moedas enviadas com sucesso!';
    } catch (err) {
      enviarMoedasMsg.textContent = err.message || 'Erro ao enviar moedas';
    }
  }
  ...
  async function carregarExtrato() {
    try {
      const data = await api.get('/professor/extrato');
      ...
      const extratoCards = data.transacoes
        .map(t => {
          const desc =
            (t.descricao || '') +
            (t.alunoNome || t.alunoEmail
              ? ` <span class='text-xs text-gray-500'>(Aluno: ${t.alunoNome || ''} ${
                  t.alunoEmail ? '- ' + t.alunoEmail : ''
                })</span>`
              : '');

          return `
            <div class="bg-white rounded shadow p-3 mb-2 border border-slate-200">
              ...
              <div class="mb-1">
                <strong>Valor:</strong>
                <span class="${t.valor < 0 ? 'text-red-600' : 'text-green-600'}">${t.valor}</span>
              </div>
              <div class="mb-1">
