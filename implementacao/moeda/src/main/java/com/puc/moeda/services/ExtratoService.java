...
public class ExtratoService {
  ...
  @Autowired private TransacaoMoedaRepository transacaoRepository;

  @Autowired private ResgateBeneficioRepository resgateRepository;
  ...
  public List<ExtratoItemDTO> consultarExtratoAluno(Aluno aluno) {
    ...
    List<TransacaoMoeda> recebimentos =
        transacaoRepository.findByDestinatarioOrderByDataTransacaoDesc(aluno);
    for (TransacaoMoeda t : recebimentos) {
      ...
      if (t.getRemetente() != null) {
        item.setProfessorNome(t.getRemetente().getNome());
        item.setProfessorEmail(t.getRemetente().getEmail());
      }
      if (t.getDestinatario() != null) {
        item.setAlunoNome(t.getDestinatario().getNome());
        item.setAlunoEmail(t.getDestinatario().getEmail());
      }
      extrato.add(item);
    }
    ...
    for (ResgateBeneficio r : resgates) {
      ...
      item.setDescricao(
          "Resgate: " + r.getBeneficio().getNome() + " (Código: " + r.getCodigoResgate() + ")");
      item.setData(r.getDataResgate());
      // Dados extras para frontend
      if (r.getBeneficio() != null && r.getBeneficio().getEmpresaParceira() != null) {
        item.setEmpresaNome(r.getBeneficio().getEmpresaParceira().getNome());
      }
      if (r.getAluno() != null) {
        item.setAlunoNome(r.getAluno().getNome());
        item.setAlunoEmail(r.getAluno().getEmail());
      }
      extrato.add(item);
    }
  ...
  public List<ExtratoItemDTO> consultarExtratoProfessor(Professor professor) {
    ...
    List<TransacaoMoeda> envios =
        transacaoRepository.findByRemetenteOrderByDataTransacaoDesc(professor);
    ...
    for (int i = 0; i < envios.size(); i++) {
      ...
      // Adiciona nome/email do aluno para o frontend
      if (t.getDestinatario() != null) {
        item.setAlunoNome(t.getDestinatario().getNome());
        item.setAlunoEmail(t.getDestinatario().getEmail());
      }
